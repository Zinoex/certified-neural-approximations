class BackupLinearization:
    ...
    def linearize_sample(self, sample):
        # First try the first method
        augmented_sample = self.method1.linearize_sample(sample)

        # If the sample is finite, return it
        if augmented_sample.isfinite():
            return augmented_sample

        # Otherwise, use the backup method
        return self.method2.linearize_sample(sample)
