...
from certified_neural_approximations.translators import TaylorTranslator, NumpyTranslator
from certified_neural_approximations.translators.taylor_translator import CertifiedFirstOrderTaylorExpansion
...
class TestCertifiedFirstOrderTaylorExpansion:
    ...
    def test_translator_to_format(self):
        te = self.translator.to_format(
            self.expansion_point, self.domain[0], self.domain[1]
        )

        assert np.array_equal(te.expansion_point, self.expansion_point)
        assert np.array_equal(te.domain[0], self.domain[0])
        assert np.array_equal(te.domain[1], self.domain[1])
        assert np.array_equal(te.linear_approximation[0], np.eye(len(self.expansion_point)))
        assert np.array_equal(te.linear_approximation[1], self.expansion_point)
        assert np.array_equal(te.remainder[0], np.zeros(len(self.expansion_point)))
        assert np.array_equal(te.remainder[1], np.zeros(len(self.expansion_point)))
    ...
    def test_pow(self):
        ...
        expected_constant = np.power(expansion_point, exponent)
        expected_jacobian = (exponent * np.power(expansion_point, exponent - 1)).reshape(-1, 1) * te.linear_approximation[0]
    ...
    def test_cbrt(self):
        ...
        expected_constant_pos = np.power(expansion_point_pos, 1/3)
        expected_jacobian_pos = ((1/3) * np.power(expansion_point_pos, -2/3)).reshape(-1, 1) * te_pos.linear_approximation[0]
        ...
        cbrt_x_pos = np.power(x_test_pos, 1/3)
        ...
        expected_constant_multi_pos = np.power(expansion_point_multi_pos, 1/3)
        expected_jacobian_multi_pos = ((1/3) * np.power(expansion_point_multi_pos, -2/3)).reshape(-1, 1) * te_multi_pos.linear_approximation[0]
        ...
        cbrt_x_multi = np.power(x_test_multi, 1/3)
