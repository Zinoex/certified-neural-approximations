...
from certified_neural_approximations.nn_dynamics import NNDynamics
...
def generate_data_from_trajectories(dynamics_model, batch_size=128, dt=0.02, trajectory_length=64, device=None):
    ...
    X_train = torch.tensor(trajectories[:, :-1, :], dtype=torch.float32).reshape(trajectory_length * batch_size, x0.size(0)).to(device)
    y_train = torch.tensor(trajectories[:, 1:, :], dtype=torch.float32).reshape(trajectory_length * batch_size, x0.size(0)).to(device)
...
def train_nn_from_trajectories(dynamics_model, learning_rate=1e-6, num_epochs=500000, batch_size=128,
    ...
    for epoch in range(num_epochs):
        if epoch % 50 == 0:
            X_train, y_train = generate_data_from_trajectories(dynamics_model, batch_size=batch_size, trajectory_length=trajectory_length, device=model.device)
