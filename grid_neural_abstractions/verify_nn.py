...
def process_batch(onnx_path, delta, L, worker_progress_counters, batch_id, data):
    """
    ...
    for idx in range(len(X_train)):
        ...
        if res == 'sat':
            ...
            for i, outputVar in enumerate(outputVars):
                if vals[outputVar] > dynamics_value[i] + delta * L or vals[outputVar] < dynamics_value[i] - delta * L:
                    violation_found = True
                    break
            assert violation_found, "No dimension violates the bound, this is not a valid counterexample"
            cex_list.append(cex)
...
def verify_nn(onnx_path, delta=0.01, mu=1.0, R=1.0, model_path="simple_nn.pth"):
    ...
    batch_selector = lambda i, batch_size: (X_train[i:i + batch_size], y_train[i:i + batch_size])
    num_samples = len(X_train)

    aggregate = lambda agg, x: agg + x

    executor = SinglethreadExecutor()
    sat_counter = executor.execute(local_process_batch, batch_selector, num_samples, aggregate)
...
if __name__ == "__main__":
    verify_nn('simple_nn.onnx', delta=0.01, mu=1.0, R=1.0, model_path="simple_nn.pth")
